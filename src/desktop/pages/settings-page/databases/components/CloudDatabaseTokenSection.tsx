  ...
  return (
    <div className={desktopStyles.DatabaseTokenSection}>
      {tokens.map((token) => (
        <div
          key={token.id}
          data-testid={TestIds.CloudDatabase.InboxTokenRow}
          className={desktopStyles.DatabaseTokenRow}
        >
          <span className={desktopStyles.DatabaseTokenLine}>
            {sectionLabel} · {token.name}
          </span>
          <div className={desktopStyles.DatabaseTokenActions}>
            <button type="button" className={desktopStyles.DatabaseTokenCopyLink} onClick={() => onCopy(token.token)}>
              {localize('common.copy', 'Copy')}
            </button>
            <span className={desktopStyles.DatabaseTokenActionSeparator}>·</span>
            <button type="button" className={desktopStyles.DatabaseTokenRevokeLink} onClick={() => onRevoke(token.id)}>
