...
export class WorkbenchSelfhostedSyncService implements ISelfhostedSyncService {
  ...
  get hasServer(): boolean {
    return this.storage !== null && this.enabled;
  }
  ...
  async addServer(config: Omit<ISelfhostedSyncServerConfig, 'id'>): Promise<string> {
    ...
    const newServer: ISelfhostedSyncServerConfig = {
      ...config,
      id: serverId,
    };
  ...
  private initServer(config: ISelfhostedSyncServerConfig | null): IDatabaseStorage | null {
    if (!config) {
      return null;
    }
    switch (config.type) {
